...
data_pornot %>% head() %>% 
  select(term, degree, college, dept, start, end, name, grade, general, keys, Attnd, concentration) %>%
...
**Data Overview:**  
```{r message = FALSE, warning = FALSE, echo = FALSE}
df_vote_college %>% head() %>%
  select(start,	end,	college,	name,	elected,	vote_support,	vote_object,	vote_invalid,	college_population) %>%
  knitr::kable() %>% kableExtra::kable_styling()
```
